...
TiGame 微信小程序现在使用 **Taro 4.2.1 + React**。旧的原生 `miniprogram/` 客户端已经删除；小程序入口负责微信能力适配，主界面和游戏业务直接编译仓库现有的 `app/page.tsx`、`app/game/*` 等共享 React/TypeScript 源码。

> **重要：这里没有使用 `<web-view>` 加载外部网页。** `app.json` 中的 `renderer: "webview"` 是微信“经典小程序渲染器”的名称，不是 `<web-view>` 组件。最终 JS、WXSS、WXML、Logo 等静态资源都进入 `dist/miniapp/` 小程序包，上传后由微信自己的小程序分发/CDN 下发；只有 HTTP API 和 WebSocket 访问 TiGame 服务器。
...
首次进入小程序时，壳层使用微信原生能力收集资料：

- `button open-type="chooseAvatar"` 选择微信头像；
- `input type="nickname"` 输入/选择微信昵称；
- 资料保留在用户自己的微信小程序本地 Storage，避免每次打开重复填写；
- 头像会先压缩为约 32–48px 的小缩略图，并限制为不超过 4096 字符的 data URL。

小程序通过 `__TIGAME_PLATFORM__.getUserProfile()` 把资料交给共享页面。创建房间时头像随 `hostAvatarData` 发送，申请加入时随 `avatarData` 发送。服务器只把头像放入对应 `GameRoom` Durable Object 的房间状态，不写入 D1/长期用户数据库。房间关闭或自动销毁时 `GameRoom` 调用 `storage.deleteAll()`，把头像、房间状态、ticket 等房间临时数据一起删除。
